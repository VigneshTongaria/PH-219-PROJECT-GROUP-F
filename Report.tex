...
The data provided is generated with Pythia 8  Monte Carlo event generator. \vspace{2 mm}\\
In the ultrarealistic nucleus to nucleus collisions, the mean transerve momentum [pT], of the emitted particles fluctuates event to event.
In this project we will study these fluctuations from event to event of the [pT] and plot them to see how they vary in the events. 
In this project we will argue that the probhability distubution of the [pT], is not gaussian but has a positive skew. 
We will also calculate intensive skewness and standard skewness of the [pT].
We will also try to argue that fluctuations in [pT]  originate from fluctuations in the total energy of the fluid.
Also these fluctuations are not dependent on the specific setup used in the experiment.\vspace{2mm}\\
...
\begin{equation}
\label{eq:5}
\mathrm{ Intensive\ Skewness\ is\ given\ by}= 
